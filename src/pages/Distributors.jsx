...
const getCurrentBalance = (distributor) =>
  Number(distributor.current_balance ?? distributor.outstanding_balance ?? 0);
...
const getTotalPurchases = (distributor) =>
  Number(distributor.total_purchases ?? distributor.purchase_total ?? 0);
...
const getTotalPayable = (distributor) =>
  Number(
    distributor.total_payable ?? Math.max(0, getCurrentBalance(distributor)),
  );
...
const getNetDistributorBalance = (distributor) =>
  Number(
    distributor.net_distributor_balance ??
      getTotalPayable(distributor) - getDistributorReceivable(distributor),
  );
...
const distributorFormData = (distributor = empty) =>
  formFields.reduce(
    (values, field) => ({ ...values, [field]: distributor[field] || "" }),
    {},
  );
...
export default function Distributors() {
  ...
  const openEditDistributor = (distributor) => {
    ...
    setForm({
      ...
      opening_balance: distributor.opening_balance ?? 0,
    });
  ...
  const save = async (e) => {
    ...
    try {
      ...
      if (editing)
        await api.put(`/distributors/${editing.id}`, {
          ...payload,
          id: editing.id,
        });
  ...
  const summary = useMemo(
    () => ({
      payable: list.reduce(
        (sum, distributor) => sum + getTotalPayable(distributor),
        0,
      ),

      receivable: list.reduce(
        (sum, distributor) => sum + getDistributorReceivable(distributor),
        0,
      ),

      netBalance: list.reduce(
        (sum, distributor) => sum + getNetDistributorBalance(distributor),
        0,
      ),

      purchases: list.reduce(
        (sum, distributor) => sum + getTotalPurchases(distributor),
        0,
      ),

      paidAdjusted: list.reduce(
        (sum, distributor) => sum + getTotalPaidAdjusted(distributor),
        0,
      ),

      active: list.filter(
        (distributor) =>
          String(getStatus(distributor) || "active").toLowerCase() === "active",
      ).length,
    }),
    [list],
  );
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      icon: Scale,
      tone: summary.netBalance < 0 ? "text-emerald-700" : "text-slate-900",
      bg: "bg-blue-50",
      iconBg: "bg-blue-100",
  ...
  return (
    <div className="space-y-7" data-testid="distributors-page">
      {/* =========================================================
          PAGE HEADER
      ========================================================= */}
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-500">
            Manage supplier profiles, purchases, outstanding balances and
            distributor ledgers from one place.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        {summaryCards.map(
          ({ label, value, icon: Icon, tone, bg, iconBg, iconTone }) => (
            <div
              key={label}
              className="
                rounded-2xl
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  {label === "Active Distributors" ? "Live" : "Summary"}
                </div>
              </div>
      ...
      >
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center justify-between gap-3 px-1 text-xs text-slate-500">
            <span>
              Showing{" "}
              <strong className="text-slate-800">{filteredList.length}</strong>{" "}
              of <strong className="text-slate-800">{list.length}</strong>{" "}
              distributors
            </span>
      ...
      {filteredList.length === 0 ? (
        ...
        >
          ...
          <h3 className="mt-4 text-lg font-bold text-slate-800">
            {search ? "No distributors found" : "No distributors yet"}
          </h3>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          {filteredList.map((d) => {
            const currentBalance = getCurrentBalance(d);

            const balance = balanceTone(currentBalance);

            const status = getStatus(d) || "Active";

            const totalPurchases = getTotalPurchases(d);

            const totalPaid = getTotalPaidAdjusted(d);

            const payable = getTotalPayable(d);

            const receivable = getDistributorReceivable(d);
              ...
              >
                ...
                <div className="grid grid-cols-1 border-b border-slate-100 sm:grid-cols-3">
                  <div className="border-b border-slate-100 p-4 sm:border-b-0 sm:border-r">
                    ...
                    >
                      {getDistributorBalanceLabel(currentBalance)}
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-3 border-t border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <span>
                      ...
                      <strong className="font-semibold text-slate-700">
                        {formatDate(getLastPurchaseDate(d))}
                      </strong>
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      "
                    >
                      <Link to={`/ledger/distributor/${d.id}`}>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <div className="mt-1 max-w-xl text-xs leading-5 text-amber-800/80">
                      Enter any existing amount already payable to this
                      distributor. Leave it at zero if there is no opening
                      balance.
                    </div>
                  </div>
                </div>

  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue="sales"><TabsList className="h-auto flex-wrap rounded-xl bg-white/70 p-1 shadow-sm"><TabsTrigger value="sales">Sales</TabsTrigger><TabsTrigger value="stock">Stock & Expiry</TabsTrigger><TabsTrigger value="outstanding">Outstanding</TabsTrigger><TabsTrigger value="analytics">Analytics</TabsTrigger></TabsList>
      ...
      <TabsContent value="analytics" className="mt-5 space-y-5">
        ...
        <div className="grid gap-5 xl:grid-cols-2"><ChartCard title="Purchase Return Value" subtitle="Return value by medicine in INR." empty={!hasValues(returnRows, ["value"])}><ResponsiveContainer><BarChart data={returnRows.slice(0, 10)}><CartesianGrid stroke="#e2e8f0" vertical={false} /><XAxis dataKey="name" /><YAxis width={70} /><Tooltip formatter={moneyTip} /><Legend /><Bar dataKey="value" name="Return value (INR)" fill="#dc2626" /></BarChart></ResponsiveContainer></ChartCard><ChartCard title="Purchase Return Status" subtitle="Returned quantity by settlement status." empty={!hasValues(returnRows, ["returnedQty"])}><ResponsiveContainer><BarChart data={returnRows.slice(0, 10)}><CartesianGrid stroke="#e2e8f0" vertical={false} /><XAxis dataKey="status" /><YAxis width={70} /><Tooltip /><Legend /><Bar dataKey="returnedQty" name="Returned quantity (units)" fill="#d4a72c" /></BarChart></ResponsiveContainer></ChartCard></div>
        <div className="grid gap-5 xl:grid-cols-2"><DataTable title="Purchase Return Analytics" subtitle="Track returned quantity, value, and settlement status." columns={["Medicine", "Qty Returned", "Value", "Status"]} rows={returnRows.slice(0, 10)} emptyText="Data not available currently." renderRow={(row) => <tr key={row.id || row.name}>{td(row.name, "font-semibold text-slate-800")}{td(row.returnedQty)}{td(fmtINR(row.value), "font-bold")}{td(row.status)}</tr>} /><DataTable title="Category Profitability" subtitle="Use category margin to guide buying and promotions." columns={["Category", "Revenue", "Profit", "Margin"]} rows={categoryRows.slice(0, 10)} emptyText="Data not available currently." renderRow={(row) => <tr key={row.category}>{td(row.category, "font-semibold text-slate-800")}{td(fmtINR(row.revenue))}{td(fmtINR(row.profit), "font-bold text-emerald-700")}{td(pct(row.margin))}</tr>} /></div>
      </TabsContent>
    </Tabs>
  </div>;
}

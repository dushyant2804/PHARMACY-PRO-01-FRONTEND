...
const normalizeOptionCollection = (responseData) => {
...
const getBatchNumber = (item) =>
  firstDefined(item.batch_number, item.batch_no, item.batch, item.batchNo, "");

const getMedicineOptionName = (item) =>
  firstDefined(item.medicine_name, item.name, item.medicine, "");

const getMedicineOptionId = (item) =>
  firstDefined(item.medicine_id, item.med_id, item.id, item.medicine_key, "");

const getDistributorId = (item) =>
  firstDefined(item.distributor_id, item.supplier_id, item.distributor?.id, "");

const getDistributorName = (item) =>
  firstDefined(item.distributor_name, item.distributor, item.supplier_name, item.distributor?.name, "");

const getExpiryValue = (item) =>
  firstDefined(item.expiry_date, item.expiry, item.expiryDate, "");

const getAvailableStock = (item) =>
  Number(firstDefined(item.available_stock, item.total_stock, item.stock, item.quantity, item.qty, 0) || 0);

const getPurchaseRate = (item) =>
  Number(firstDefined(item.purchase_rate, item.purchase_price, item.rate, item.cost_price, 0) || 0);

const buildBatchKey = (item, index = 0) => [
  getMedicineOptionId(item),
  getMedicineOptionName(item),
  getBatchNumber(item),
  getExpiryValue(item),
  getDistributorId(item),
  index,
].map((value) => String(value || "")).join("|");

const normalizeMedicineSuggestions = (responseData) => {
  const medicines = normalizeOptionCollection(responseData);

  return medicines.flatMap((medicine, medicineIndex) => {
    const batches = firstDefined(medicine.batches, medicine.batch_entries, medicine.stocks, medicine.inventory_batches, []);

    if (Array.isArray(batches) && batches.length > 0) {
      return batches.map((batch, batchIndex) => {
        const merged = { ...medicine, ...batch };
        const option = {
          medicine_id: getMedicineOptionId(merged),
          medicine_key: firstDefined(merged.medicine_key, merged.key, getMedicineOptionId(merged), ""),
          name: getMedicineOptionName(merged),
          batch_number: getBatchNumber(merged),
          expiry_date: getExpiryValue(merged),
          available_stock: getAvailableStock(merged),
          distributor_id: getDistributorId(merged),
          distributor_name: getDistributorName(merged),
          purchase_rate: getPurchaseRate(merged),
          mrp: firstDefined(merged.mrp, merged.selling_price, ""),
        };

        return {
          ...option,
          key: buildBatchKey(option, `${medicineIndex}-${batchIndex}`),
        };
      });
    }

    const option = {
      medicine_id: getMedicineOptionId(medicine),
      medicine_key: firstDefined(medicine.medicine_key, medicine.key, getMedicineOptionId(medicine), ""),
      name: getMedicineOptionName(medicine),
      batch_number: getBatchNumber(medicine),
      expiry_date: getExpiryValue(medicine),
      available_stock: getAvailableStock(medicine),
      distributor_id: getDistributorId(medicine),
      distributor_name: getDistributorName(medicine),
      purchase_rate: getPurchaseRate(medicine),
      mrp: firstDefined(medicine.mrp, medicine.selling_price, ""),
    };

    return [{ ...option, key: buildBatchKey(option, medicineIndex) }];
  }).filter((option) => option.name || option.batch_number);
};

const formatMedicineOption = (option) => (
  `${option.name || "Unknown"} | Batch: ${option.batch_number || "—"} | Exp: ${option.expiry_date || "—"} | Stock: ${option.available_stock ?? 0} | Distributor: ${option.distributor_name || "—"} | Rate: ${fmtINR(option.purchase_rate || 0)}`
);
...
export default function PurchaseReturns() {
  ...
  const searchMedicines = async (value) => {
    ...
    try {
      ...
      setMedicineSuggestions(normalizeMedicineSuggestions(data));
    } catch (e) {
      toast.error(formatApiError(e));
      setMedicineSuggestions([]);
    } finally {
  ...
  const selectMedicineSuggestion = (option) => {
    setForm({
      ...form,
      medicine_id: option.medicine_id || "",
      medicine_key: option.medicine_key || option.medicine_id || "",
      selected_batch_key: option.key || "",
      medicine_name: option.name || "",
      batch_number: option.batch_number || "",
      expiry_date: option.expiry_date || "",
      distributor_id: option.distributor_id ? String(option.distributor_id) : "",
  ...
  const handleMedicineKeyDown = (event) => {
    ...
    if (event.key === "ArrowDown") {
      ...
      setActiveSuggestionIndex((current) => Math.min(current + 1, medicineSuggestions.length - 1));
      return;
    }
    ...
    if (event.key === "Enter" || event.key === "Tab") {
      const option = medicineSuggestions[activeSuggestionIndex];
      if (!option) return;
      event.preventDefault();
      selectMedicineSuggestion(option);
    }
  };
  ...
  const validateForm = () => {
    ...
    if (!form.selected_batch_key) return "Select the exact medicine batch being returned";
    ...
    if (form.available_stock !== "" && Number(form.return_quantity) > Number(form.available_stock)) {
      return "Return quantity cannot exceed available batch stock";
    }
  ...
  const submit = async (e) => {
    ...
    const payload = {
      ...
      medicine_id: form.medicine_id || undefined,
      medicine_key: form.medicine_key || undefined,
      medicine_name: form.medicine_name.trim(),
      batch_number: form.batch_number.trim(),
      expiry_date: form.expiry_date,
      available_stock: form.available_stock === "" ? undefined : Number(form.available_stock),
            ...
            <div className="grid md:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select value={form.distributor_id} onValueChange={updateDistributor}>
                  <SelectTrigger className="rounded-sm mt-1"><SelectValue placeholder="Auto-filled after batch selection" /></SelectTrigger>
                  <SelectContent>
                    {distributors.map((distributor) => (
                      <SelectItem key={distributor.id} value={String(distributor.id)}>
                        {distributor.name}
              ...
              <div className="relative md:col-span-2">
                ...
                <Input
                  ...
                  onFocus={() => form.medicine_name && setShowMedicineSuggestions(true)}
                  placeholder="Search medicine and select exact batch"
                  className="rounded-sm mt-1"
                  required
                />
                {showMedicineSuggestions && (medicineSuggestions.length > 0 || medicineSearchLoading) && (
                  <div className="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-sm shadow max-h-64 overflow-y-auto">
                    {medicineSearchLoading && (
                      <div className="p-3 text-sm text-slate-500">Searching medicines…</div>
                    )}
                    {!medicineSearchLoading && medicineSuggestions.map((option, index) => (
                      <button
                        key={option.key}
                        type="button"
                        ...
                        onClick={() => selectMedicineSuggestion(option)}
                      >
                        {formatMedicineOption(option)}
                      </button>
                    ))}
                  </div>
                )}
                {form.selected_batch_key && (
                  <div className="mt-2 text-xs text-slate-500">
                    Selected batch stock: <span className="font-semibold text-slate-700">{form.available_stock || 0}</span>
                    {form.mrp !== "" && <> · MRP: <span className="font-semibold text-slate-700">{fmtINR(form.mrp)}</span></>}
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <Input
                  type="number"
                  min="0"
                  max={form.available_stock === "" ? undefined : Number(form.available_stock)}
                  step="1"
                ...
                <div className="text-xs text-slate-500 mt-1">
                  Available batch stock: <span className="font-semibold text-slate-700">{form.available_stock === "" ? "Select a batch" : form.available_stock}</span>
                </div>
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  className="rounded-sm mt-1 bg-slate-50"
                  readOnly={Boolean(form.selected_batch_key)}
                  required
                />
              </div>

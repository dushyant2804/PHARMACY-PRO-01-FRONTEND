...
const getLedgerTxnDate = (transaction) => {
  ...
  return isOpeningBalance
    ? transaction.opening_balance_date ||
        transaction.date ||
...
const getTransactionMode = (transaction) =>
  transaction?.payment_mode || transaction?.mode;
...
const isPurchaseTransaction = (transaction) =>
  ...
  getTransactionKind(transaction).includes("purchase");
    ...
    ? [
        {
          value: "purchase",
          label: "Purchase",
          description: "Add a supplier bill",
          Icon: ArrowUpRight,
          tone: "red",
        },
        {
...
const getAdjustedAgainstItems = (transaction) =>
  Array.isArray(transaction?.adjusted_against)
    ? transaction.adjusted_against.filter(
        (item) => item && hasFieldValue(item.amount),
      )
    : [];
...
export const normalizeCustomerMonthlySummary = (payload) => {
  ...
  return rows.map((row) => ({
    ...
    paymentsReceived: toLedgerNumber(
...
export const hasCustomerMonthlySummary = (payload) =>
  normalizeCustomerMonthlySummary(payload).length > 0;
...
export default function Ledger() {
  ...
  if (!data)
    return <div className="text-slate-500">No ledger data was returned.</div>;
    ...
    type === "distributor"
      ? transactions.filter((transaction) => {
          ...
          const matchesType =
            ...
            getTransactionKind(transaction) === ledgerTypeFilter;
          const matchesQuery =
            !query ||
            [
  ...
  const currentFinancialYear =
    data.current_financial_year || getCurrentIndianFinancialYear();
  const financialYearOptions = [
    ...new Set([
      currentFinancialYear,
      ...(data.available_financial_years || [])
        .map(getCleanFieldValue)
        .filter(Boolean),
    ]),
  ];
  ...
  const showCustomerMonthlySummary =
    type === "customer" && hasNonZeroCustomerMonthlySummary(data);
  ...
  const handleEditSave = async (e) => {
    ...
    try {
      const editingIsOpeningBalance =
        type === "distributor" &&
        isOpeningBalanceTransaction(editingTransaction);
      ...
      await api.patch(
        `/distributor-transactions/${editingTransaction.id}`,
        payload,
      );
  ...
  const downloadLedger = async () => {
    ...
    try {
      const response = await api.get(`/ledger/${type}/${id}/export`, {
        params:
          type === "distributor"
            ? getDistributorLedgerParams(selectedFinancialYear)
            : undefined,
        responseType: "blob",
      });
      const contentType =
  ...
  const editingIsOpeningBalance =
    ...
    editingTransaction &&
    isOpeningBalanceTransaction(editingTransaction);
  const editingIsPurchase =
    editingTransaction && isPurchaseTransaction(editingTransaction);
  const editingDate = editingIsOpeningBalance
    ? editForm.date
    : editingTransaction
      ? String(
          (type === "distributor"
            ? getLedgerTxnDate(editingTransaction)
  ...
  const ledgerWhatsappUrl = whatsappUrl(
    entity.phone,
    ledgerShareMessage({
      type,
      entity,
  ...
  const isDistributorSpecificFinancialYear =
    type === "distributor" && selectedFinancialYear !== ALL_FINANCIAL_YEARS;
  const broughtForwardBalance = Number(data.brought_forward_balance || 0);
  const showBroughtForwardBox =
    isDistributorSpecificFinancialYear && broughtForwardBalance !== 0;
  const broughtForwardFromFinancialYear =
    getCleanFieldValue(data.brought_forward_from_financial_year) ||
    "previous FY";
  const isClosedFinancialYear = data.is_financial_year_closed === true;
  const showClosingBalanceBox =
    isDistributorSpecificFinancialYear &&
    data.is_financial_year_closed !== undefined &&
    data.is_financial_year_closed !== null;
  ...
  const ledgerSummaryTitle =
    selectedFinancialYear === ALL_FINANCIAL_YEARS
      ? "All Ledger Summary"
      ...
      ? [
          {
            ...
            valueClassName:
              ledgerSummaryAdjustments > 0 ? "text-blue-600" : "text-slate-700",
          },
        ]
      : []),
    {
      ...
      valueClassName:
        Number(data.balance || 0) > 0 ? "text-red-600" : "text-emerald-600",
    },
  ];
    ...
    <div className="space-y-6" data-testid="ledger-page">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          {entity.phone && (
            <div className="text-sm text-slate-500 mt-1">{entity.phone}</div>
          )}
        </div>
        <div className="text-right space-y-2">
          <div>
            ...
            <div
              className={`font-heading text-3xl font-bold font-mono-nums ${
                displayedBalance > 0 ? "text-red-600" : "text-emerald-600"
              }`}
            >
            ...
            <div className="text-xs text-slate-500">
              {type === "distributor" ? distributorBalanceLabel : "Receivable"}
            </div>
          ...
          <div className="flex flex-wrap justify-end gap-2">
            ...
            <a
              href={ledgerWhatsappUrl || undefined}
              target="_blank"
              rel="noreferrer"
              aria-disabled={!ledgerWhatsappUrl}
              title={
                ledgerWhatsappUrl
                  ? "Share ledger summary on WhatsApp"
        ...
        {type === "distributor" && (
          <div className="w-full sm:w-[220px]">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setSelectedFinancialYear(normalizeFinancialYear(value))
              }
            >
              <SelectTrigger
                className="rounded-sm mt-1 bg-white"
                data-testid="financial-year-filter"
              >
                <SelectValue />
              </SelectTrigger>
        ...
        <div className="bg-white border border-slate-200 rounded-sm p-4 space-y-4">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            ...
            <div className="rounded-sm border border-blue-100 bg-blue-50 p-4">
              ...
              <div
                className={`mt-1 text-2xl font-bold font-mono-nums ${
            ...
            <div className="border rounded-sm overflow-hidden">
              ...
              <div className="divide-y max-h-[260px] overflow-auto">
                {monthlyPurchases.length ? (
                  monthlyPurchases.map((transaction) => (
                    <div
                      key={transaction.id}
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-slate-500">
                          {fmtDate(getLedgerTxnDate(transaction))}
                        </div>
                      </div>
            ...
            <div className="border rounded-sm overflow-hidden">
              ...
              <div className="divide-y max-h-[260px] overflow-auto">
                {monthlyPayments.length ? (
                  monthlyPayments.map((transaction) => (
                    <div
                      key={transaction.id}
                      className="p-3 flex justify-between gap-3 text-sm"
                    >
      ...
      {showCustomerMonthlySummary && (
        <div
          className="bg-white border border-slate-200 rounded-sm p-4 space-y-4"
          data-testid="customer-monthly-summary"
        >
          <div>
            <div className="text-xs uppercase tracking-[0.15em] font-semibold text-slate-500">
              Monthly summary
            ...
            <p className="text-sm text-slate-500">
              Credit sales, payments received, and closing balances from the
              customer ledger API response.
            </p>
          </div>
              ...
              <tbody>
                {customerMonthlySummary.map((row) => (
                  <tr key={row.month}>
                    ...
                    <td className="num-cell font-semibold text-red-600">
                      {fmtINR(row.creditSales)}
                    </td>
                    <td className="num-cell font-semibold text-emerald-600">
                      {fmtINR(row.paymentsReceived)}
                    </td>
                    <td
                      className={`num-cell font-bold ${row.netMovement > 0 ? "text-red-600" : "text-emerald-600"}`}
                    >
                      {fmtINR(row.netMovement)}
                    </td>
                    <td
                      className={`num-cell font-bold ${row.closingBalance > 0 ? "text-red-700" : "text-emerald-700"}`}
                    >
      ...
      {showBroughtForwardBox && (
        <div className="rounded-sm border border-blue-100 bg-blue-50/60 px-4 py-3 sm:px-5 sm:py-4 shadow-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div
              className={`font-heading text-xl sm:text-2xl font-bold font-mono-nums ${broughtForwardBalance > 0 ? "text-red-600" : "text-emerald-600"}`}
            >
        ...
        <div className="flex flex-col gap-3 rounded-sm border border-slate-200 bg-white p-3 sm:flex-row">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={
                type === "customer"
                  ? "Search invoice, reference, notes, or payment mode"
                  : "Search reference, invoice, notes, or payment mode"
              }
              className="rounded-sm pl-9"
            />
          </div>
          <Select value={ledgerTypeFilter} onValueChange={setLedgerTypeFilter}>
            <SelectTrigger className="w-full rounded-sm sm:w-[180px]">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <table className="data-table">
          ...
          <tbody>
            ...
            {ledgerTransactions.map((t) => {
              ...
              const isDistributorPurchase =
                isDistributorLedger && isPurchaseTransaction(t);
              const billStatusDisplay = isDistributorPurchase
                ? getBillStatusDisplay(t)
                : null;
              const billWiseInfo = isDistributorPurchase
                ? getBillWiseInfo(t)
                : [];
              const adjustedAgainstItems = isDistributorLedger
                ? getAdjustedAgainstItems(t)
                : [];

              return (
                <tr
                  key={t.id}
                  className={billStatusDisplay?.rowClassName || undefined}
                >
                  ...
                  <td className="uppercase text-xs tracking-wider font-semibold">
                    <div className="flex flex-col items-start gap-1">
                      <span
                        className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${getTransactionKind(t) === "payment" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : isDistributorPurchase ? "border-red-200 bg-red-50 text-red-700" : "border-slate-200 bg-slate-50 text-slate-600"}`}
                      >
                        {getTransactionTypeLabel(t)}
                      </span>
                      {billStatusDisplay && (
                        <span
                          className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${billStatusDisplay.badgeClassName}`}
                        >
                  ...
                  <td>
                    ...
                    {type === "customer" && (t.invoice_id || t.invoice?.id) && (
                      <Link
                        to={`/invoices/${t.invoice_id || t.invoice.id}`}
                        className="mt-1 inline-flex text-xs font-semibold text-blue-600 hover:underline"
                      >
                        View invoice{" "}
                        {t.invoice_number || t.invoice_reference || "detail"}
                      </Link>
                    )}
                    {type === "customer" &&
                      !(t.invoice_id || t.invoice?.id) &&
                      (t.invoice_number || t.invoice_reference) && (
                      ...
                      <div className="mt-2 text-xs text-slate-600">
                        ...
                        <div className="mt-1 space-y-0.5">
                          {adjustedAgainstItems.map((item, index) => {
                            const invoiceRef = getFirstAvailableValue([
                              item.invoice_no,
                              item.transaction_id,
                            ]);
                  ...
                  <td className="text-xs uppercase">
                    {getTransactionMode(t) || "—"}
                  </td>
                  <td
                    className={`num-cell font-semibold ${getTransactionKind(t) === "payment" ? "text-emerald-600" : "text-slate-800"}`}
                  >
                    {getTransactionKind(t) === "payment"
                      ? "−"
                      : Number(t.amount || 0) < 0
                        ? ""
                        : "+"}
                  ...
                  <td
                    className={`num-cell font-bold ${Number(t.running_balance || 0) > 0 ? "text-red-700" : "text-emerald-700"}`}
                  >
                    {fmtINR(t.running_balance)}
                  </td>
                  <td>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {ledgerSummaryItems.map((item) => (
              ...
              >
                ...
                <div
                  className={`mt-2 text-xl sm:text-2xl font-bold font-mono-nums break-words ${item.valueClassName}`}
                >
                  {fmtINR(item.value)}
                </div>
              </div>
      ...
      {showClosingBalanceBox && (
        <div className="rounded-sm border border-slate-200 bg-slate-50 px-4 py-3 sm:px-5 sm:py-4 shadow-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div
              className={`font-heading text-xl sm:text-2xl font-bold font-mono-nums ${Number(closingBalanceValue || 0) > 0 ? "text-red-600" : "text-emerald-600"}`}
            >
          ...
          <form onSubmit={handleEditSave} className="space-y-3">
            ...
            <div>
              ...
              <Input
                value={
                  editingTransaction
                    ? getTransactionTypeLabel(editingTransaction)
                    : ""
                }
                disabled
            ...
            <div>
              ...
              <Input
                value={
                  editingTransaction ? fmtINR(editingTransaction.amount) : ""
                }
                disabled
                className="rounded-sm mt-1 bg-slate-100"
              />
              ...
              <>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, bill_number: e.target.value })
                    }
                    className="rounded-sm mt-1"
                  />
                </div>
              ...
              <>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setEditForm({ ...editForm, payment_mode: v, mode: v })
                    }
                  >

...
export default function Login() {
  ...
  return <div className="login-shell min-h-screen grid lg:grid-cols-[1.18fr_.82fr]">
    <section className="relative hidden lg:flex min-h-screen overflow-hidden p-12 xl:p-16 flex-col justify-between text-white">
      ...
      <div className="relative z-10 max-w-xl pb-14"><div className="premium-kicker">Intelligence for modern pharmacy</div><h1 className="mt-5 font-heading text-5xl xl:text-6xl font-extrabold leading-[1.02] tracking-tight">Every medicine.<br/>Every movement.<br/><span className="gold-text">Perfectly managed.</span></h1><p className="mt-6 max-w-lg text-base leading-relaxed text-emerald-50/65">A precise, GST-ready operating system for inventory, billing, purchase intelligence and patient trust.</p><div className="mt-8 flex gap-5 text-xs text-emerald-50/65"><span className="flex gap-2"><ShieldCheck className="w-4 text-amber-300"/>Secure by design</span><span className="flex gap-2"><CheckCircle2 className="w-4 text-amber-300"/>Built for India</span></div></div>
    ...
    <section className="relative flex items-center justify-center bg-[#f5f3ed] p-5 sm:p-10"><div className="absolute inset-0 login-paper"/><div className="relative w-full max-w-md rounded-[28px] border border-white/70 bg-white/85 p-7 sm:p-9 shadow-[0_30px_80px_rgba(10,35,29,.14)] backdrop-blur-xl"><div className="lg:hidden mb-8"><BrandLogo /></div><div className="premium-kicker text-emerald-800">Secure console access</div><h2 className="mt-3 font-heading text-4xl font-extrabold tracking-tight text-slate-950">Welcome back</h2><p className="mt-2 text-sm text-slate-500">Enter your credentials to continue to PharmacyOS.</p>
      ...
      <div className="mt-5 rounded-xl border border-amber-200/70 bg-amber-50/60 p-3 text-xs text-slate-600"><div className="flex justify-between gap-3"><b className="text-amber-900">Explore demo</b><button type="button" onClick={()=>{setEmail("admin@pharmacy.com");setPassword("admin123");}} className="font-semibold text-emerald-700" data-testid="use-demo-login">Use demo account</button></div><div className="mt-1 font-mono text-[11px]">admin@pharmacy.com / admin123</div></div><div className="mt-6 text-center text-xs text-slate-500">New to PharmacyOS? <Link to="/signup" className="font-bold text-emerald-700">Create account</Link></div></div></section>
  </div>;
}

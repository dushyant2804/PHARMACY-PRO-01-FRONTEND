  ...
  return <div className="dashboard-shell">
    ...
    <section className="grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
      <div className="dashboard-panel p-5"><SectionHeader eyebrow="Inventory watch" title="Low stock medicines" action="Open inventory" onAction={() => navigate("/inventory")} />
        <div className="grid gap-2 sm:grid-cols-2">{lowStockItems.slice(0, 8).map((item, index) => <div key={item.id || index} onClick={() => navigate("/inventory")} className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-3 text-left transition hover:border-orange-200 hover:shadow-sm"><span className="min-w-0"><span className="block truncate text-sm font-semibold text-slate-700">{getMedicineName(item)}</span><span className="mt-1 block text-xs text-slate-500">{firstDefined(item.qty, getAvailableStock(item), 0)} units left</span></span><LowStockWorkflowControl compact item={item} onUpdated={(updated) => setData((current) => ({ ...current, low_stock_items: lowStockItems.map((row) => (row.id || row.medicine_id) === (updated.id || updated.medicine_id) ? updated : row) }))} /></div>)}{!lowStockItems.length && <p className="col-span-full rounded-xl bg-emerald-50 p-4 text-sm text-emerald-700">Stock levels look healthy today.</p>}</div>
      </div>
